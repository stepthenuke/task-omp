...
class IService {
 ...
 protected:
  explicit IService(const std::string& name) : name_{name} {}

 protected:
  std::string name_;
};

// Interface for a service with single connection, object, and interface (name =
// interface). More concretely -- base for both SharingService and
// RegisteringService
